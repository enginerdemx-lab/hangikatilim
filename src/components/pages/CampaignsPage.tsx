...
export const CampaignsPage: React.FC<CampaignsPageProps> = ({ onNavigate }) => {
  ...
  // Mock Data for Campaigns based on real companies
  const campaigns = [
    {
      id: 1,
      company: "Sinpaş Yapı Tasarruf",
      logo: "https://hangikatilim.com/images/5-simpas.png",
      campaignImage: "https://images.unsplash.com/photo-1533473359331-0135ef1bcfb0?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
      badge: "Faizsiz Fırsat",
      badgeColor: "text-orange-600 border-orange-200 bg-orange-50",
      title: "Peşinatsız Araç Finansmanı",
      orgFeeRate: "İndirimli",
      features: [
        "Çekilişli sistem avantajı",
        "Peşinat zorunluluğu yok",
        "120 aya varan vade seçenekleri"
      ],
      link: "https://www.sinpasyts.com/?utm_source=adwords&utm_campaign=searchtextev|gorselyok|duyarlimetin|offeryok|turkiye|search|brandinghgp|ev&utm_medium=search"
    },
    {
      id: 2,
      company: "Eminevim",
      logo: "https://hangikatilim.com/images/1-eminevim.png",
      campaignImage: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
      badge: "Özel Kampanya",
      badgeColor: "text-blue-600 border-blue-200 bg-blue-50",
      title: "5. Ayda Kesin Teslim Konut",
      orgFeeRate: "%20 İndirim",
      features: [
        "Faizsiz, Vade Farksız",
        "Değer artışına karşı korumalı",
        "Erken teslimat garantisi"
      ],
      link: "https://www.eminevim.com"
    },
    {
      id: 3,
      company: "Fuzul Ev",
      logo: "https://hangikatilim.com/images/2-fzlev.png",
      campaignImage: "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
      badge: "Sponsorlu",
      badgeColor: "text-purple-600 border-purple-200 bg-purple-50",
      title: "Çekilişsiz Hızlı Teslimat",
      orgFeeRate: "Sabit Ücret",
      features: [
        "Yarısı peşin yarısı taksit",
        "Kefilsiz şartsız başvuru",
        "Türkiye'nin her yerinden ev seçimi"
      ],
      link: "https://www.fuzul.com.tr"
    },
    {
      id: 4,
      company: "Birevim",
      logo: "https://hangikatilim.com/images/6-birevim.png",
      campaignImage: "https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
      badge: "Fırsat",
      badgeColor: "text-green-600 border-green-200 bg-green-50",
      title: "Tasarrufa İlk Adım Paketi",
      orgFeeRate: "Özel Oran",
      features: [
        "Küçük taksitlerle başlama imkanı",
        "Ara ödeme zorunluluğu yok",
        "Organizasyon ücretinde taksit imkanı"
      ],
      link: "https://www.birevim.com"
    }
  ];
               ...
               {campaigns.map((camp) => (
                  <div key={camp.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden hover:shadow-lg transition-all duration-300 group">
                     ...
                     <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                        <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${camp.badgeColor}`}>
                           <Zap size={12} /> {camp.badge}
                        </div>
                     ...
                     <div className="p-5 flex flex-col md:flex-row gap-6 items-center">
                        ...
                        <div className="w-full md:w-32 flex-shrink-0 flex flex-col items-center justify-center text-center">
                           <div className="w-20 h-20 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center mb-2 shadow-sm border border-gray-100 dark:border-slate-700 p-1">
                              <img src={camp.logo} alt={camp.company} className="w-full h-full object-contain" />
                           </div>
                           <h3 className="font-bold text-xs text-gray-800 dark:text-gray-200">{camp.company.split(' ')[0]}</h3>
                        </div>

                        {/* NEW Campaign Image Section */}
                        <div className="hidden md:block w-40 h-28 flex-shrink-0 rounded-lg overflow-hidden shadow-sm border border-gray-100 dark:border-slate-700 group-hover:shadow-md transition-all">
                            <img src={camp.campaignImage} alt={camp.title} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500" />
                        </div>
                        ...
                        <div className="flex-1 text-center md:text-left border-l-0 md:border-l border-gray-100 dark:border-slate-700 md:pl-6 w-full">
                           ...
                           <div className="flex items-center justify-center md:justify-start gap-4 mb-3 text-sm text-gray-600 dark:text-gray-400">
                              <div className="flex flex-col">
                                 <span className="text-xs text-gray-400">Org. Ücreti</span>
                                 <span className="font-bold text-gray-900 dark:text-white">{camp.orgFeeRate}</span>
                              </div>
                              <div className="w-px h-8 bg-gray-200 dark:bg-slate-600"></div>
                              <div className="flex flex-col">
                                 <span className="text-xs text-gray-400">Vade</span>
                                 <span className="font-bold text-gray-900 dark:text-white">{months} Ay</span>
                              </div>
                              <div className="w-px h-8 bg-gray-200 dark:bg-slate-600"></div>
                              <div className="flex flex-col">
                                 <span className="text-xs text-gray-400">Tutar</span>
                                 <span className="font-bold text-gray-900 dark:text-white">{formatMoney(amount)} TL</span>
                              </div>
                           </div>
                        ...
                        <div className="w-full md:w-auto flex-shrink-0 flex flex-col gap-2">
                           <button 
                              className="w-full bg-white dark:bg-slate-700 border-2 border-[#ff6f00] text-[#ff6f00] hover:bg-[#ff6f00] hover:text-white font-bold py-2.5 px-6 rounded-lg transition-all text-sm"
                              onClick={handleCalculate}
                           >
                              Hemen Başvur
                           </button>
                        ...
                        >
                           <div className="flex justify-between items-start mb-2">
                              ...
                              <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border ${
                                  plan.systemType === SystemType.LOTTERY 
